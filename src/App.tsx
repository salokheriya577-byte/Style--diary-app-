...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsubAuth = onAuthStateChanged(auth, async (u) => {
      ...
      if (u) {
        ...
        const itemsUnsub = onSnapshot(query(collection(db, `${userPath}/items`), orderBy('createdAt', 'desc')), (snap) => {
          const items = snap.docs.map(doc => ({ id: doc.id, ...doc.data() } as ClothingItem));
          setData(prev => ({ ...prev, items }));
        });
        
        const outfitsUnsub = onSnapshot(query(collection(db, `${userPath}/outfits`), orderBy('createdAt', 'desc')), (snap) => {
          const outfits = snap.docs.map(doc => ({ id: doc.id, ...doc.data() } as Outfit));
          setData(prev => ({ ...prev, outfits }));
        });
        
        const logsUnsub = onSnapshot(query(collection(db, `${userPath}/logs`), orderBy('createdAt', 'desc')), (snap) => {
          const logs = snap.docs.map(doc => ({ id: doc.id, ...doc.data() } as LogEntry));
          setData(prev => ({ ...prev, logs }));
        });

        unsubs.push(itemsUnsub, outfitsUnsub, logsUnsub);
      } else {
  ...
  const toggleItemFavorite = async (item: ClothingItem) => {
    if (!auth.currentUser) return;
    const userPath = `users/${auth.currentUser.uid}`;
    try {
      await setDoc(doc(db, `${userPath}/items`, item.id), { isFavorite: !item.isFavorite }, { merge: true });
    } catch (err) {
      console.error("Firestore Error:", err);
    }
  };

  const toggleOutfitFavorite = async (outfit: Outfit) => {
    if (!auth.currentUser) return;
    const userPath = `users/${auth.currentUser.uid}`;
    try {
      await setDoc(doc(db, `${userPath}/outfits`, outfit.id), { isFavorite: !outfit.isFavorite }, { merge: true });
    } catch (err) {
      console.error("Firestore Error:", err);
    }
  };
  ...
  const deleteItem = async (id: string) => {
    ...
    try {
      ...
      batch.delete(doc(db, `${userPath}/items`, id));
      
      // Cleanup items in outfits logic... 
      // Note: Realistically I should update all outfits that contains this item id
      // But for now just deleting the item is enough as Firestore listeners will update everything.
      // Wait, I should really use batch to update outfits too if I want consistency.
      data.outfits.forEach(o => {
        if (o.itemIds.includes(id)) {
          const newItems = o.itemIds.filter(iid => iid !== id);
          if (newItems.length === 0) {
            batch.delete(doc(db, `${userPath}/outfits`, o.id));
          } else {
            batch.update(doc(db, `${userPath}/outfits`, o.id), { itemIds: newItems });
          }
        }
      });
  ...
  const deleteOutfit = async (id: string) => {
    ...
    try {
      const batch = writeBatch(db);
      batch.delete(doc(db, `${userPath}/outfits`, id));
      
      data.logs.forEach(l => {
        if (l.outfitId === id) {
          batch.delete(doc(db, `${userPath}/logs`, l.id));
        }
      });
  ...
  const handleLogOutfit = async (outfitId: string, customDate?: Date) => {
    ...
    if (currentOutfit && currentOutfit.itemIds.length > 1) {
      ...
      const recentLogs = data.logs.filter(log => {
        ...
        const diff = Math.abs(differenceInDays(targetDate, logDate));
    ...
    try {
      await setDoc(doc(db, `${userPath}/logs`, logId), {
        date: dateStr,
        outfitId,
  ...
  const handleCreateAndLogOutfit = async (newOutfitData: Omit<Outfit, 'id'>, customDate?: Date) => {
    if (!auth.currentUser) return;
    const userPath = `users/${auth.currentUser.uid}`;
    ...
    try {
      const batch = writeBatch(db);
      batch.set(doc(db, `${userPath}/outfits`, outfitId), {
        ...newOutfitData,
        userId: auth.currentUser.uid,
        createdAt: serverTimestamp()
      });
      const logId = Math.random().toString(36).substr(2, 9);
      batch.set(doc(db, `${userPath}/logs`, logId), {
  ...
  const handleUnlogOutfit = async (logId: string) => {
    if (!auth.currentUser) return;
    const userPath = `users/${auth.currentUser.uid}`;
    try {
      await deleteDoc(doc(db, `${userPath}/logs`, logId));
    } catch (err) {
      console.error("Firestore Error:", err);
    }
  };
  ...
  const savePreferences = async (preferences: string[]) => {
    if (!auth.currentUser) return;
    try {
      await setDoc(doc(db, 'users', auth.currentUser.uid), { preferences }, { merge: true });
    } catch (err) {
      console.error("Firestore Error:", err);
    }
  };
            ...
            <main className="flex-1 overflow-y-auto px-4 py-6">
              <AnimatePresence mode="wait">
                {activeTab === 'journal' && <JournalView 
                  ...
                  repetitionAlert={repetitionAlert} 
                  setRepetitionAlert={setRepetitionAlert} 
                  showWowOverlay={showWowOverlay} 
                  setShowWowOverlay={setShowWowOverlay}
                  onLogOutfit={handleLogOutfit}
                  onUnlogOutfit={handleUnlogOutfit}
                ...
                {activeTab === 'wardrobe' && <WardrobeView 
                  ...
                  data={data} 
                  setData={setData} 
                  onEditOutfit={handleEditOutfitRequest} 
                  onToggleItemFav={toggleItemFavorite} 
                  onToggleOutfitFav={toggleOutfitFavorite} 
...
function JournalView({ 
  ...
  setShowLogModal: (b: boolean) => void;
  repetitionAlert: any;
  setRepetitionAlert: (a: any) => void;
  showWowOverlay: boolean;
  setShowWowOverlay: (b: boolean) => void;
  onLogOutfit: (id: string) => void;
  onUnlogOutfit: (id: string) => void;
  onCreateAndLogOutfit: (o: any) => void;
  key?: string 
}) {
  ...
  return (
    ...
    >
      ...
      {/* Gemini AI Trigger */}
      <AiSuggestionPanel data={data} onUseOutfit={onLogOutfit} onCreateAndUseOutfit={onCreateAndLogOutfit} />
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {data.items.slice(0, 10).map((item) => (
                      <button
                        key={item.id}
                        onClick={() => onCreateAndLogOutfit({ name: `${item.name} Look`, itemIds: [item.id] })}
...
function WardrobeView({ data, setData, onEditOutfit, onToggleItemFav, onToggleOutfitFav, onDeleteItem, onDeleteOutfit, onLogOutfit, onCreateAndLogOutfit }: { data: UserData; setData: React.Dispatch<React.SetStateAction<UserData>>; onEditOutfit: (o: Outfit) => void; onToggleItemFav: (i: ClothingItem) => void; onToggleOutfitFav: (o: Outfit) => void; onDeleteItem: (id: string) => void; onDeleteOutfit: (id: string) => void; onLogOutfit: (id: string) => void; onCreateAndLogOutfit: (o: Omit<Outfit, 'id'>) => void; key?: string }) {
  ...
  const handleAddItem = async (e: React.FormEvent) => {
    ...
    try {
      if (editingItem) {
        await setDoc(itemRef, { ...itemData, updatedAt: serverTimestamp() }, { merge: true });
      } else {
        await setDoc(itemRef, { ...itemData, createdAt: serverTimestamp() });
      }
      setIsAdding(false);
      setEditingItem(null);
  ...
  const deleteItem = async (id: string) => {
    ...
    try {
      ...
      // Also cleanup outfits containing this item
      data.outfits.forEach(o => {
        if (o.itemIds.includes(id)) {
          const newItemIds = o.itemIds.filter(itemId => itemId !== id);
          if (newItemIds.length === 0) {
            batch.delete(doc(db, `${userPath}/outfits`, o.id));
          } else {
            batch.update(doc(db, `${userPath}/outfits`, o.id), { itemIds: newItemIds });
          }
        }
      });
                ...
                <div>
                   ...
                   <div className="flex items-center justify-between mt-2">
                     ...
                     <div className="flex gap-1 opacity-100 transition-opacity">
                        ...
                        <button 
                          onClick={() => onToggleOutfitFav(outfit)}
                          className={cn(
...
function StudioView({ data, setData, initialOutfit, onClearInitial }: { data: UserData; setData: React.Dispatch<React.SetStateAction<UserData>>; initialOutfit?: Outfit | null; onClearInitial?: () => void; key?: string }) {
  ...
  const handleSaveOutfit = async () => {
    if (!outfitName || selectedItemIds.length === 0 || !auth.currentUser) return;
    
    const userPath = `users/${auth.currentUser.uid}`;
    const outfitId = editingOutfitId || Math.random().toString(36).substr(2, 9);
    const outfitRef = doc(db, `${userPath}/outfits`, outfitId);

    const outfitData = {
      name: outfitName,
      itemIds: selectedItemIds,
      userId: auth.currentUser.uid,
      isFavorite: false // Default to false or keep existing
    };
    ...
    try {
      if (editingOutfitId) {
        ...
        await setDoc(outfitRef, { ...outfitData, createdAt: serverTimestamp() });
      }
      setOutfitName('');
      setSelectedItemIds([]);
    } catch (err) {
...
function StatsView({ data, setData, score, user, onEditOutfit, onToggleItemFav, onToggleOutfitFav, onDeleteItem, onDeleteOutfit, onLogOutfit, onCreateAndLogOutfit }: { data: UserData; setData: React.Dispatch<React.SetStateAction<UserData>>; score: number; user: any; onEditOutfit: (o: Outfit) => void; onToggleItemFav: (i: ClothingItem) => void; onToggleOutfitFav: (o: Outfit) => void; onDeleteItem: (id: string) => void; onDeleteOutfit: (id: string) => void; onLogOutfit: (id: string) => void; onCreateAndLogOutfit: (o: Omit<Outfit, 'id'>) => void; key?: string }) {
